    ...
    while (std::getline(file, line)) { // read each line
        ...
        for (char c : line) { // get the tile value from each character
            ...
            if (buffer == MapTile::Error) {
                std::cerr << "Error: Invalid character '" << c << "' in map file " << filename << std::endl;
                file.close();
                return false;
            }
...
bool map_reader::find_player(map_t &map, int &x, int &y, MapTile &dir) {
    for (int j = 0; j < map.height; j++) {
        for (int i = 0; i < map.width; i++) {
            if (map.data[j][i] == MapTile::Player_North || map.data[j][i] == MapTile::Player_South ||
                map.data[j][i] == MapTile::Player_West || map.data[j][i] == MapTile::Player_East) {
                x = i;
                y = j;
                dir = map.data[j][i];
