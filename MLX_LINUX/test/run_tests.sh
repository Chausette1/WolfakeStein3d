...
test_default_main(){
    ...
    wid1=$(xdotool search --name "MLX-Title1" | head -n 1)
    if [ -z "$wid1" ]; then
        log_error "Window with title 'Title1' not found!"
        exit 1
    fi
    
    wid2=$(xdotool search --name "MLX-Title2")
    if [ -z "$wid2" ]; then
        log_error "Window with title 'Title2' not found!"
        exit 1
    fi
    
    wid3=$(xdotool search --name "MLX-Title3")
    if [ -z "$wid3" ]; then
        log_error "Window with title 'Title3' not found!"
        exit 1
    fi
